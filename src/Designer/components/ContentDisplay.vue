...
//        selectItemContainerStyle: {},
    ...
    computed: {
      ...
      contentStyle() {
        return {
          width: warpUnit(
            this.scaleValue * this.containerWidth +
            CONTENT_OFFSET * 2,
          ),
          height: warpUnit(
            this.scaleValue * this.containerHeight +
            CONTENT_OFFSET * 2,
          ),
        };
      },
    ...
    methods: {
      ...
      calculateAdsorbentValue(value, directionOffset, direction) {
        ...
        const anotherValueOffsetObj = this.getOffsetValue(anotherValue, testSet, moveVector);
        let offsetConfig;
//        direction === 'hoz' && console.info(valueOffsetObj, anotherValueOffsetObj, moveVector);
        if (valueOffsetObj.isHitTarget && anotherValueOffsetObj.isHitTarget) {
          offsetConfig = Math.abs(valueOffsetObj.offset) < Math.abs(anotherValueOffsetObj.offset) ? valueOffsetObj : anotherValueOffsetObj;
      ...
      getOffsetValue(value, set = [], vector) {
        ...
        set.forEach(test => {
          ...
          if (currentOffset * vector >= 0 && Math.abs(currentOffset) < Math.abs(offset)) {
  ...
  <div class="work-space" ref="workspace">
    ...
    >
      ...
      <div class="grid-guide">
        <!--辅助线-->
        <div :class="['grid-guide-item']" v-for="n in gridGuideHozSet" :key="n" :style="{top: `${n}px`, width: `${Math.max(containerWidth, containerHeight)}px`}"></div>
        <div :class="['grid-guide-item ver']" v-for="n in gridGuideVerSet" :key="'ver' + n" :style="{left: `${n}px`, width: `${Math.max(containerWidth, containerHeight)}px`}"></div>
        <div v-if="this.highlightState && highlightGuideConfig.ver.guide !== null" class="highlight grid-guide-item" :style="{top: `${highlightGuideConfig.ver.guide}px`, width: `${Math.max(containerWidth, containerHeight)}px`}"></div>
        <div v-if="this.highlightState && highlightGuideConfig.hoz.guide !== null" class="highlight grid-guide-item ver" :style="{left: `${highlightGuideConfig.hoz.guide}px`, width: `${Math.max(containerWidth, containerHeight)}px`}"></div>
      </div>
    </div>
