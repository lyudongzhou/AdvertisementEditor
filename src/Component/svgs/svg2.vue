<template lang="html">
  <baseCmp :cmpConfig="cmpConfig">
    <div style="width:100%;height:100%" ref="svgContainer"></div>
  </baseCmp>
</template>
...
// import "./tmp";
const aColor = ["#dd684f", "#5855c8", "#b12621", "#f9a326", "#f9a326"];
const aPath = [
  ...
  "M46,3H3.64A3.41,3.41,0,0,0,0,6.28V35.42a3.67,3.67,0,0,0,3.64,3.64h9.6a1.79,1.79,0,0,1,1.33,3l-3.31,3.31A1.81,1.81,0,0,0,12.91,48H37.08a1.81,1.81,0,0,0,1.66-2.65L35.43,42a1.78,1.78,0,0,1,1.32-3H46a3.67,3.67,0,0,0,3.64-3.64V6.28A3.61,3.61,0,0,0,46,3Zm-5,29.14H7.61a1.86,1.86,0,0,1-1.65-2,1.56,1.56,0,0,1,1.65-1.65H41.05a1.56,1.56,0,0,1,1.66,1.65A1.86,1.86,0,0,1,41.05,32.11Z",
  "M32.75,6.42H14.25A3.09,3.09,0,0,0,11.17,9.5V32.63a3.09,3.09,0,0,0,3.08,3.08h18.5a3.09,3.09,0,0,0,3.08-3.08V9.5A3.09,3.09,0,0,0,32.75,6.42ZM35.83.25H11.17A6.19,6.19,0,0,0,5,6.42v37a6.19,6.19,0,0,0,6.17,6.17H35.83A6.19,6.19,0,0,0,42,43.42v-37A6.19,6.19,0,0,0,35.83.25Zm3.09,43.17a3.09,3.09,0,0,1-3.09,3.08H11.17a3.09,3.09,0,0,1-3.09-3.08v-37a3.09,3.09,0,0,1,3.09-3.08H35.83a3.09,3.09,0,0,1,3.09,3.08ZM24.27,38.8a2.31,2.31,0,1,0,2.31,2.31A2.36,2.36,0,0,0,24.27,38.8Z",
  "M8,34.22V16.65C8,7.18,11,.22,23.53.22c10.8,0,14.36,4.38,14.36,12.47,0,.3.07.6.07.84H27c0-3.89-.28-5.75-3.42-5.75s-3.76,2.1-3.76,9.65v4.32c1.12-1.68,3.83-2.58,7-2.7C35.73,18.87,39,25.11,39,33c0,10.19-3.55,17.15-15.4,17.15C12.53,50.17,8,43.46,8,34.22Zm19.3.36c0-4.74-.07-7.86-3.7-7.86a4.81,4.81,0,0,0-3.76,1.92v7.92c0,3.3.77,6.23,3.9,6.23S27.3,40,27.3,34.58Z",
  "M21.15.21h9.7L49,47.11H37.36L33.93,37H18.07L14.64,47.11H3Zm4.91,13.51h-.23l-5.71,17H31.76Z",
];
export default {
  name: "svgMaskCmp",
  props: ["cmpConfig"],
  ...
  computed: {
    // tmpSvg(){
    //   return get(SVGS)[type](this.cmpConfig);
    // }
  },
  watch: {
    "cmpConfig.layoutConfig.width"() {
      this.arrDraw.forEach((ele) => {
        ele.size(
          this.cmpConfig.layoutConfig.width,
          this.cmpConfig.layoutConfig.height
        );
      });
    },
    ...
    "cmpConfig.props.url"(value) {
      this.image.node.setAttribute("href", value);
    },
  },
  mounted() {
    const type = this.cmpConfig.props.type.split(".")[1] || 0;
    this.arrDraw = [];
    var draw = SVG().addTo(this.$refs.svgContainer).size("100%", "100%");
    var path1 = draw.path(aPath[type]).fill(aColor[type]);
    this.arrDraw.push(path1);
    var path = draw.path(aPath[type]);
    var image = (this.image = draw.image(this.cmpConfig.props.url).attr({
      preserveAspectRatio: "none",
    }));
    var clip = draw.clip().add(path);
    image.clipWith(clip);
    this.arrDraw.push(path);
    this.arrDraw.push(image);
    this.arrDraw.forEach((ele) => {
      ele.size(
        this.cmpConfig.layoutConfig.width,
        this.cmpConfig.layoutConfig.height
